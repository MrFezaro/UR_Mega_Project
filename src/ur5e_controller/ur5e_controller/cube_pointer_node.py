...
class State(Enum):
    ...
    SEARCH   = auto()   # rotate 45 degrees and look again
...
class CubePointerNode(Node):
    ...
    def __init__(self):
        ...
        # Camera intrinsics, no calibration file, set these manually.
        # For 640x480 with ~70 deg FOV: fx=fy=500. Tune if XY position is off.
    ...
    def _pixel_to_robot(self, u: float, v: float, tcp_xyz):
        ...
        ray_cam  = np.array([(u - cx) / fx, (v - cy) / fy, 1.0]) # camera frame
        ray_base = R_tool @ R_cam @ ray_cam                        # rotate to robot base

        if abs(ray_base[2]) < 1e-6 or (table_z - tcp_xyz[2]) / ray_base[2] < 0:
            return None

        t  = (table_z - tcp_xyz[2]) / ray_base[2]  # ray-plane intersection
        pt = tcp_xyz + t * ray_base                 # world position
    ...
    def _run(self):
        ...
        # Search poses: rotate home XY around base Z axis, keep same height
        ...
        while rclpy.ok():
            ...
            # SEARCH
                ...
                if search_idx >= len(search_poses):
                    ...
                    self.get_logger().info(f'{color}: searching at {deg:+.0f} deg')
